-- AlterTable: add accessToken as nullable first (8 existing rows need a value)
ALTER TABLE "Order" ADD COLUMN     "accessToken" TEXT;

-- Backfill existing orders with a random token (core Postgres functions only, no extension needed)
UPDATE "Order" SET "accessToken" = md5(random()::text || clock_timestamp()::text || id) WHERE "accessToken" IS NULL;
